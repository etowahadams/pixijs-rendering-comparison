...
export class ZoomableScatterplot {
  ...
  private prevZoomLevel: number = 1.0;
  ...
  constructor(width: number, height: number, container: HTMLDivElement) {
    ...
    this.data = generateRandomData(1000); // You can specify the number of points you want
    // this.data = [
    //   { x: 100, y: 100 },
    // ];
  ...
  private drawData(mousepositionX: number): void {
    ...
    this.data.forEach((point) => {
      ...
      // point.x = point.x * (this.zoomLevel / this.prevZoomLevel);
  ...
  private calculateUpdatedX(originalX: number, mouseX: number): number {
    ...
    const scale = translate * (this.zoomLevel);
    return scale + (mouseX);
  }
